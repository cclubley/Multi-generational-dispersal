...
# interest for post-processing in order to reduce the size of the data. I keep 
...
# Using these variables I now create an individual netCDF file for each of the
# planktonic larval durations of interest: 14, 17, 25 and 28 days.

# Create a .csv file for a 28 day PLD -------------------------------------

# As I was using a high performance computer to run these scripts, I print the
# PLD so that I can keep track of the script progress whilst it is running.
print("28 day PLD")

# As each particle has a unique 'death' date, I fill any NA values in each 
...
# For each particle in each variable, I extract the first row of data which 
...
# seconds). The PLD I am interested in is 28 days, which is equal to 672 rows. 
# So to get the sink information for each particle, I extract the 672nd row.
lat_last <- lat[672, ]
lon_last <- lon[672, ]
time_last  <- time[672, ]
dist_last <- dist[672, ]

# I then want to bind the particle source information together into one data 
# frame. To do this I first convert each variable to a data frame
...
# I give each particle a number starting from 1 to the number of rows in the 
# data frame
first$Particle <- paste(1:nrow(first)) 
# I label the data by assigning a column called 'Connectivity' and labelling 
...
# I then repeat the exact same process for the sink data
...
# I combine the information for the particle source and the particle sink into
# one data frame using rbind
mydata <- rbind(first, last)

# Then I sort the data by the particle column so that the source and sink 
# information for each particle is displayed together
mydata <- mydata[with(mydata, order(Particle)), ]

# Finally I convert the time to a datetime object using the origin time I 
# defined earlier - this isn't necessary, but I find it more intuitive than the 
# seconds format
mydata$Time <- as.POSIXct(mydata$Time, origin=time_origin)

# Finally, I write the .csv file
write.csv(mydata, "./csv file/2000_28_Random_100000_passive_2D_1deg_settled.csv", row.names = FALSE)

# I then remove the sink information from the environment ('last'), BUT I keep 
# the source information ('first') as this will remain the same regardless of 
# the PLD  
rm(last)

# I repeat the exact same process as above for the remaining PLDs (25, 17 and 
