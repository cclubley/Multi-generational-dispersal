...
setwd("D:/OneDrive back up (27-02-2024)/Chapters/(3) Dispersal Limitation/(3) Methods/Modelling/Original release file/Shapefiles/Country Shapefiles")
...
setwd("D:/OneDrive back up (27-02-2024)/Chapters/(3) Dispersal Limitation/(3) Methods/Modelling/Original release file/Shapefiles")
...
EUR_sf <- st_as_sf(EUR)
SpPolygon_sf <- st_as_sf(SpPolygon)

EUR_sf <- st_transform(EUR_sf, crs = "+proj=longlat +datum=WGS84 +no_defs")

EUR_sf <- st_cast(EUR_sf, "MULTIPOLYGON")
SpPolygon_sf <- st_cast(SpPolygon_sf, "MULTIPOLYGON")

#ocean_shp <- rgeos::gDifference(SpPolygon, EUR)
ocean_shp_sf <- st_difference(SpPolygon_sf, EUR_sf)

#plot(ocean_shp)
plot(st_geometry(ocean_shp_sf))
...
st_write(ocean_shp_sf, "oceans.shp")
