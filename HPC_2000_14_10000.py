...
def get_settlement_cells(landmask):
    mask_lap = np.roll(landmask, -16, axis=0) + np.roll(landmask, 16, axis=0) 
    mask_lap += np.roll(landmask, -16, axis=1) + np.roll(landmask, 16, axis=1)
    mask_lap -= 4*landmask
    settlement = np.ma.masked_array(landmask, mask_lap > 0) 
    settlement = settlement.mask.astype('int')
    return settlement
settlement = get_settlement_cells(landmask)
fieldset.add_field(Field('settlement', settlement, lon=fieldset.U.grid.lon, lat=fieldset.U.grid.lat, mesh='spherical'))
...
kernels = pset.Kernel(AdvectionRK4)+ pset.Kernel(Particle_death)+ pset.Kernel(SampleAge)+ pset.Kernel(Sample_land)+ pset.Kernel(Sample_settlement)+pset.Kernel(Unbeaching)+pset.Kernel(Distance)+pset.Kernel(DiffusionUniformKh)+pset.Kernel(Settle_death)

output_file = pset.ParticleFile(name="2000_Random_10000_10_4_14_passive_2D_1deg_output.nc", outputdt=delta(hours=1))
